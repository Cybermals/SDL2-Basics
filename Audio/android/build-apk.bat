...
javac -classpath C:/android-sdk/platforms/android-28/android.jar src/org/libsdl/app/*.java src/ml/cybermals/Audio/*.java -d obj
...
aapt add tmp.apk lib/armeabi-v7a/libSDL2_mixer.so
aapt add tmp.apk lib/armeabi-v7a/libmpg123.so
...
aapt add tmp.apk lib/arm64-v8a/libSDL2_mixer.so
aapt add tmp.apk lib/arm64-v8a/libmpg123.so
aapt add tmp.apk lib/arm64-v8a/libmain.so
zipalign 4 tmp.apk SDL2Audio.apk
cd ..

rem Sign the apk
echo Signing...
call apksigner sign --ks my-release-key.keystore tmp/SDL2Audio.apk
echo done